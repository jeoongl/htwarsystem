...
$insert_visitors_query = "INSERT INTO eco_attraction_visitors_tbl (name, gender, age, tourist_type, citizenship, address, phone, email, bookers_id)
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
$stmt = $conn->prepare($insert_visitors_query);

for ($i = 0; $i < count($names); $i++) {
    $visitor_name = htmlspecialchars($names[$i]);
    $visitor_gender = htmlspecialchars($genders[$i]);
    $visitor_age = intval($ages[$i]);
    $visitor_type = htmlspecialchars($types[$i]);
    $visitor_citizenship = htmlspecialchars($citizenships[$i]);
    $visitor_address = htmlspecialchars($addresses[$i]);
    $visitor_mobile = htmlspecialchars($mobiles[$i]);
    $visitor_email = htmlspecialchars($emails[$i]);

    // Bind and execute for each passenger
    $stmt->bind_param("ssisssssi", $visitor_name, $visitor_gender, $visitor_age, $visitor_type, $visitor_citizenship, $visitor_address, $visitor_mobile, $visitor_email, $reservation_id);
    $stmt->execute();
}
    ...
    $registration_price_query = "SELECT registration_price, environmental_fee FROM registration_prices_tbl WHERE business_id = ?";
    $stmt = $conn->prepare($registration_price_query);
    $stmt->bind_param("i", $business_id);
    $stmt->execute();
    $stmt->bind_result($registration_price, $environmental_fee);
    $stmt->fetch();
    $stmt->close();

    $total_env_fee = $environmental_fee * $num_people;
    $total_reg_fee = $registration_price * $num_people;
    $total_payment = $total_env_fee + $total_reg_fee; // Use boat_price
...
$stmt->close();
    ...
    $pdf->Cell(0, 8, 'Registration Fee: PHP ' . number_format($registration_price, 2) . ' x ' . $num_people . ' = PHP ' . number_format($total_reg_fee, 2), 0, 1);
    ...
    try {
        ...
        // Email content
        $mail->isHTML(true);
        $mail->Subject = 'Registration Created!';
        $mail->Body = '
        <p>You have registered at ' . $business_name . '. Please find the registration details attached in the PDF.</p>
        ...
        $mail->AltBody = 'You have registered at ' . $business_name . '. 
