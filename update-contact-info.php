...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($count > 0) {
        ...
        $query = "UPDATE business_embeds_tbl SET contact_no = ?, email = ?, updated_by = ? WHERE business_id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssii", $contact_no, $email, $user_id, $business_id);
        ...
        $query = "INSERT INTO business_embeds_tbl (business_id, contact_no, email, updated_by) VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("issi", $business_id, $contact_no, $email, $user_id);
