...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $updated_by = $_SESSION['user_id']; // Assuming the logged-in user's ID is stored in the session
    ...
    if ($count > 0) {
        ...
        $query = "UPDATE business_embeds_tbl SET facebook = ?, instagram = ?, x_twitter = ?, linkedin = ?, updated_by = ? WHERE business_id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssssii", $facebook, $instagram, $x_twitter, $linkedin, $updated_by, $business_id);
        ...
        $query = "INSERT INTO business_embeds_tbl (business_id, facebook, instagram, x_twitter, linkedin, updated_by) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("issssi", $business_id, $facebook, $instagram, $x_twitter, $linkedin, $updated_by);
